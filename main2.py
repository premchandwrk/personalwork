...
def insert_data_into_bigquery():
    ...
    rows_to_insert = convert_to_rows(data_list)
    new_rows_to_insert = check_existing_rows(rows_to_insert)

    if new_rows_to_insert:
        # dataset_id = 'your_dataset_id'  # Replace 'your_dataset_id' with your dataset ID
        table_id = "bcs_sp_ui_all_info1"  # Replace 'your_table_id' with your table ID
        ...
        # Check for existing rows before insertion to ensure idempotence
        existing_rows = check_existing_rows(new_rows_to_insert)
        rows_to_insert = [row for row in new_rows_to_insert if row not in existing_rows]
        ...
        if rows_to_insert:
            ...
            if errors == []:
                ...
                return jsonify({'message': 'Encountered errors while inserting data.', 'errors': errors}), 500
        else:
